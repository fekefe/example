...
public class Class4a {
	...
	public static void main(String[] args) {
		...
		int[] mergeArray = mergeMethod(array1, array2);
	...
	private static int[] mergeMethod(int[] array1, int[] array2) {
		...
		while (n >= 0) {
			if (i >= 0 && j >= 0) {
				if (array1[i] > array2[j]) {
					mergeArray[n] = array1[i];
					n--;
					i--;
				} else if (array1[i] < array2[j]) {
					mergeArray[n] = array2[j];
					n--;
					j--;
				} else if (array1[i] == array2[j]) {
					mergeArray[n] = array1[i];
					mergeArray[n - 1] = array2[j];
					n = n - 2;
					i--;
					j--;
				}
